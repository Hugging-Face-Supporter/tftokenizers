...
from tftokenizers.model import compare_models
...
def test_bert_base_cased():
    ...
    out = compare_models(pretrained_model_name, [s1, s2, s3])
    np.testing.assert_array_almost_equal(
        x=out["tf_output"].numpy(),
        y=out["hf_output"].numpy(),
    )
...
def test_kb_bert_base_swedish_cased():
    ...
    out = compare_models(pretrained_model_name, [s1, s2, swe])
    np.testing.assert_array_almost_equal(
        x=out["tf_output"].numpy(),
        y=out["hf_output"].numpy(),
    )
